                ...
                <div class="table-responsive">
                    <table id="zero_config" class="table table-striped table-bordered">
                        ...
                        <tr>
                            <td>{{$key + 1}}</td>
                            <td>{{$author->name}}</td>
                            <td><img src="{{ Storage::disk('public')->url('author/'.$author->image) }}" style="width: 80px; height: 70px"></td>
                            <td>{{str_limit($author->about,'25')}}</td>
                            <td>{{$author->created_at}}</td>
                            <td>{{$author->updated_at}}</td>
                            <td class="text-center">
                                <a href="{{route('author.edit',$author->id)}}" class="btn btn-info waves-effect" >
                                    <i class="material-icons">edit</i>
                                </a>
                                <button class="btn btn-danger waves-effect" type="button" onclick="deleteCategory({{$author->id }})">
                                    <i class="material-icons">delete</i>
                                </button>
                                <form id="delete-form-{{$author->id }}" action="{{ route('author.destroy',$author->id) }}" method="POST" style="display: none;">
                                    @csrf
                                    @method('DELETE')
                                </form>
