        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <a href="{{route('contactinfo.create')}}"><button class="btn btn-danger mt-5 mb-5">Add Contact Info</button></a>
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table id="zero_config" class="table table-striped table-bordered">
                                    ...
                                    @foreach($contacts as $key=>$contact)
                                        <tr>
                                            <td>{{$key + 1}}</td>
                                            <td>{{$contact->l_title}}</td>
                                            <td>{{$contact->r_title}}</td>
                                            <td>{{$contact->r_phone}}</td>
                                            <td>{{$contact->r_email}}</td>
                                            <td>{{$contact->r_address}}</td>
                                            ...
                                            <td class="text-center">
                                                ...
                                                <form id="delete-form-{{$contact->id }}" action="{{ route('contactinfo.destroy',$contact->id) }}" method="POST" style="display: none;">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
